...
proof -
  have pi2: "1 < pi/2" using pi_gt3 by simp
  have key: "sin u \<le> sin 1" if "0 \<le> u" "u \<le> (1::real)" for u
    using sin_monotone_2pi_le[of u 1] that pi2 by simp
  show ?thesis
  proof (cases "0 \<le> t")
    case True
    then have "\<bar>sin t\<bar> = sin t"
      using assms pi_gt3 sin_ge_zero[of t] by simp
    then show ?thesis using key[of t] True assms by simp
  next
    case False
    then have "\<bar>sin t\<bar> = sin (-t)"
      using assms pi_gt3 sin_ge_zero[of "-t"] by (simp add: sin_minus)
    then show ?thesis using key[of "-t"] False assms by simp
  qed
qed
...
proof -
  ...
  then obtain \<xi> where \<xi>: "\<xi> \<in> {x<..<y}" "norm (cos y - cos x) \<le> norm (- sin \<xi> * (y - x))"
    by auto
  have "\<bar>\<xi>\<bar> \<le> 1" using \<xi>(1) assms(2,3) by auto
  ...
  finally show ?thesis by (smt (verit) abs_minus_commute)
qed
...
proof -
  have "\<bar>cos w - cos dottie\<bar> \<le> sin 1 * \<bar>w - dottie\<bar>"
    using cos_contraction[OF assms dottie_in_pm1] .
  then show ?thesis using facts(3) by simp
qed
...
proof -
  from assms obtain m where "n = Suc m" using not0_implies_Suc by force
...
next
  ...
  next
    case False then show ?thesis using cos_funpow_in_pm1[of n y0] by simp
  qed
...
proof -
  ...
  proof (rule tendsto_sandwich[OF _ _ tendsto_const null])
    show "\<forall>\<^sub>F n in sequentially. 0 \<le> \<bar>(cos ^^ n) y0 - dottie\<bar>" by simp
    show "\<forall>\<^sub>F n in sequentially. \<bar>(cos ^^ n) y0 - dottie\<bar> \<le> (sin 1)^n * \<bar>y0 - dottie\<bar>"
      using cos_funpow_bound[OF assms] by simp
  qed
...
proof
  assume "algebraic dottie"
  then have alg: "algebraic (complex_of_real dottie)" by simp
  have "complex_of_real dottie \<noteq> 0" using facts(1) by simp
  from transcendental_cos[OF alg this]
  have "\<not> algebraic (cos (complex_of_real dottie))" .
  moreover have "cos (complex_of_real dottie) = complex_of_real dottie"
    using facts(3) by (simp add: cos_of_real)
