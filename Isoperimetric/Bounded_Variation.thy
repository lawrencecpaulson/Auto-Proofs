...
lemma absolutely_integrable_bounded_variation_eq:
  ...
  shows \<open>f absolutely_integrable_on {a..b} \<longleftrightarrow>
    ...
    (\<lambda>t. integral {a..t} f) has_bounded_variation_on {a..b}\<close>
...
proof
  ...
  have bv: \<open>(\<lambda>t. integral {a..t} f) has_bounded_variation_on {a..b}\<close>
    unfolding has_bounded_variation_on_def has_bounded_setvariation_on_def
  proof (intro exI allI impI)
    ...
    proof (rule sum.cong[OF refl])
      ...
      obtain c e where ke: \<open>k = {c..e}\<close> \<open>c \<le> e\<close>
        using division_ofD(4)[OF div kd] by (auto simp: box_real)
      ...
      have \<open>integral {a..e} f = integral {a..c} f + integral {c..e} f\<close>
        using integral_combine[OF ce_sub(1) \<open>c \<le> e\<close> fint_ae] sorry
    ...
    proof (rule sum_mono)
      fix k assume \<open>k \<in> d\<close>
      obtain c e where ke: \<open>k = {c..e}\<close> \<open>c \<le> e\<close>
        using division_ofD(4)[OF div \<open>k \<in> d\<close>] by (auto simp: box_real)
      ...
      have fint: \<open>f integrable_on k\<close>
        using integrable_on_subinterval[OF int] ksub by auto
...
next
  assume R: ?R
  then have int: \<open>f integrable_on {a..b}\<close>
    and bv: \<open>(\<lambda>t. integral {a..t} f) has_bounded_variation_on {a..b}\<close>
    by auto
  show ?L
    unfolding absolutely_integrable_on_def
  proof (intro conjI)
    show \<open>f integrable_on {a..b}\<close> by (rule int)
    show \<open>(\<lambda>x. norm (f x)) integrable_on {a..b}\<close>
    proof (rule bounded_variation_absolutely_integrable_real_interval[OF int])
      fix d assume ddiv: \<open>d division_of {a..b}\<close>
      from bv obtain B where B: \<open>\<And>d t. d division_of t \<Longrightarrow> t \<subseteq> {a..b} \<Longrightarrow>
      ...
      proof (rule sum.cong[OF refl])
        fix k assume kd: \<open>k \<in> d\<close>
        obtain c e where ke: \<open>k = {c..e}\<close> \<open>c \<le> e\<close>
          using division_ofD(4)[OF ddiv kd] by (auto simp: box_real)
